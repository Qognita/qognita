...
import { Search, Loader2, AlertCircle, CheckCircle, Shield, User, LogOut } from 'lucide-react'
...
export function AnalysisForm({ onAnalysisStart, onAnalysisComplete, isAnalyzing }: AnalysisFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAuthenticated && !hasShownFirstTimeModal) {
          ...
          <>
            <Card className="bg-white/5 backdrop-blur-sm border-white/10">
              ...
              <CardContent>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(['auto', 'wallet', 'program', 'token', 'transaction'] as const).map((type) => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${addressType === type
                ...
                {!isAuthenticated ? (
                  ...
                  <div className="mt-6 flex items-center justify-between p-3 bg-slate-700 rounded-lg">
            ...
            <FirstTimeModal
              ...
              onContinueAsGuest={() => {
                localStorage.setItem('qognita_has_seen_first_time_modal', 'true')
                setHasShownFirstTimeModal(true)
                // Trigger analysis after modal closes
