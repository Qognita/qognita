...
export class SolanaService {
  ...
  async getTokenMetadata(mintAddress: string): Promise<any> {
    try {
      ...
      try {
        const tokenListResponse = await fetchWithTimeout(
  ...
  async getTokenHolders(
    mintAddress: string,
    limit: number = 100
  ): Promise<Array<{ address: string; amount: string; percentage: number }>> {
    try {
      const mintPublicKey = new PublicKey(mintAddress);
      ...
      return holders.sort((a, b) => Number(b.amount) - Number(a.amount)).slice(0, limit);
    } catch (error) {
