...
export interface SecurityVulnerability {
  type: 'REENTRANCY' | 'INTEGER_OVERFLOW' | 'MISSING_SIGNER_CHECK' | 'ACCESS_CONTROL' | 'DESERIALIZATION_ERROR' | 'AUTHORITY_ABUSE' | 'UNINITIALIZED_ACCOUNT'
...
export class SmartContractScanner {
    ...
    try {
      ...
      return {
        ...
        vulnerabilities: [{
          type: 'ACCESS_CONTROL',
          severity: 'MEDIUM',
          description: 'Could not complete security analysis - program may have restricted access',
  ...
  private async performBehavioralAnalysis(programId: string, accountInfo: any): Promise<SecurityVulnerability[]> {
    const vulnerabilities: SecurityVulnerability[] = []
    ...
    if (accountInfo.owner.toString() === 'BPFLoaderUpgradeab1e11111111111111111111111') {
      vulnerabilities.push({
        ...
        severity: 'HIGH',
        description: 'Program is upgradeable - code can be changed by program authority without notice',
    ...
    reentrancyPatterns.forEach((pattern, index) => {
      ...
      if (matches && matches.length > 0) {
        vulnerabilities.push({
          type: 'REENTRANCY',
          severity: 'CRITICAL',
          description: 'Potential reentrancy vulnerability detected - external call made before state update',
    ...
    signerPatterns.forEach((pattern, index) => {
      ...
      if (matches && matches.length > 0) {
        vulnerabilities.push({
          ...
          severity: 'CRITICAL',
          description: 'Functions performing sensitive operations without proper signer verification',
    ...
    deserializationPatterns.forEach((pattern, index) => {
      ...
      if (matches && matches.length > 0) {
        vulnerabilities.push({
          ...
          severity: 'MEDIUM',
          description: 'Unsafe deserialization detected - using unwrap() without proper error handling',
  ...
  private async performHeuristicAnalysis(programId: string, accountInfo: any): Promise<SecurityVulnerability[]> {
    const vulnerabilities: SecurityVulnerability[] = []
    ...
    vulnerabilities.push({
      type: 'ACCESS_CONTROL',
      severity: 'MEDIUM',
      description: 'Source code not available for static analysis - security cannot be fully verified',
  ...
  private async tryGitHubSource(programId: string): Promise<{ found: boolean, code?: string, metrics?: any }> {
    // This would implement GitHub API search for program source code
    // For now, return not found
    return { found: false }
  }

  private async analyzeProgramHistory(programId: string): Promise<{ ageInDays: number, hasRecentUpgrades: boolean }> {
  ...
  private determineRiskLevel(score: number, vulnerabilities: SecurityVulnerability[]): 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' {
