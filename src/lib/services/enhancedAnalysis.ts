...
export class EnhancedAnalysisService {
  ...
  async analyzeProgram(programId: string): Promise<ProgramAnalysis> {
    ...
    try {
      ...
      const isUpgradeable =
        accountInfo.owner.toString() === 'BPFLoaderUpgradeab1e11111111111111111111111';
  ...
  async analyzeTransaction(signature: string): Promise<TransactionAnalysis> {
    ...
    try {
      ...
      if (involvedPrograms.includes('11111111111111111111111111111112')) {
        ...
        const hasTransferInstruction = transaction.transaction.message.instructions.some(
          (instruction: any) => {
            if (
      ...
      if (
        involvedPrograms.includes('JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4') ||
        involvedPrograms.includes('whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc') || // Whirlpool
        involvedPrograms.includes('9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM')
      ) {
        // Serum DEX
      ...
      if (
        transaction.transaction.message.instructions.some((instruction: any) => {
          return (
            'parsed' in instruction &&
  ...
  private async analyzeTransactionPatterns(
    tokenAddress: string
  ): Promise<{ suspiciousPatterns: boolean }> {
  ...
  private async analyzeLiquidity(
    tokenAddress: string
  ): Promise<{ locked: boolean; ownershipRenounced: boolean }> {
  ...
  private async simulateTrading(
    tokenAddress: string
  ): Promise<{ canBuy: boolean; canSell: boolean }> {
