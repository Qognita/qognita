...
export class TokenScraperService {
  ...
  private async getComprehensiveHolders(
    mintAddress: string,
    mintInfo: any
    ...
    try {
      ...
      for (let i = 0; i < Math.min(tokenAccounts.length, 50); i += batchSize) {
        ...
        console.log(
          `📊 Processed ${processedCount}/${Math.min(tokenAccounts.length, 50)} accounts`
        );
      ...
      console.log(
        `✅ Found ${holders.length} holders out of ${tokenAccounts.length} total accounts`
      );
