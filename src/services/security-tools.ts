  ...
  try {
    ...
    if (tokenInfo.mintAuthority && tokenInfo.mintAuthority !== '11111111111111111111111111111111') {
      riskFactors.push({
        ...
        impact: 'The creator can mint unlimited new tokens at any time, potentially devaluing your holdings to zero'
      })
    }

    // Check 2: Freeze Authority Still Active  
    if (tokenInfo.freezeAuthority && tokenInfo.freezeAuthority !== '11111111111111111111111111111111') {
      riskFactors.push({
        level: 'CRITICAL',
        category: 'Freeze Authority',
        message: 'Freeze authority is still active',
        impact: 'The creator can freeze your tokens, making them completely unsellable and untransferable'
      })
    }
    ...
    if (liquidity && liquidity < 10000) {
      riskFactors.push({
        ...
        impact: 'Low liquidity makes it difficult to sell tokens and increases price manipulation risk'
      })
    }
    ...
    try {
      ...
      if (holdersData && Array.isArray(holdersData.holders) && holdersData.holders.length > 0) {
        ...
        const top10Holdings = holdersData.holders.slice(0, 10).reduce((sum: number, holder: any) => sum + (holder.amount || 0), 0)
    ...
    if (riskFactors.length === 0) {
      summary = 'No obvious honeypot patterns detected. However, always do your own research before investing.'
    } else {
    ...
    return {
      ...
      summary: 'Security analysis incomplete due to technical issues. Please verify token safety through other means.'
    }
  }
}
