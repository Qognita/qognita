...
function generateChartData(tokenomics: any): TokenomicsChartData {
    ...
    const distribution: TokenDistribution[] = tokenomics.distribution.map((item: any, index: number) => ({
        ...item,
...
function parseLockPeriod(period: string): number {
    ...
    if (lower.includes('30 days') || lower.includes('1 month')) return 30
    if (lower.includes('90 days') || lower.includes('3 months')) return 90
    if (lower.includes('180 days') || lower.includes('6 months')) return 180
    if (lower.includes('1 year') || lower.includes('365 days')) return 365
    return 30 // default
}
