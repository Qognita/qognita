...
async function executeWithFallback<T>(
  ...
  for (let i = 0; i < RPC_ENDPOINTS.length; i++) {
    ...
    try {
      ...
      console.warn(
        `RPC endpoint ${RPC_ENDPOINTS[(currentRpcIndex + i) % RPC_ENDPOINTS.length]} failed:`,
        error
      );
      ...
      if (error instanceof SyntaxError && error.message.includes('Unexpected token')) {
        console.error(
          `RPC returned HTML instead of JSON from ${RPC_ENDPOINTS[(currentRpcIndex + i) % RPC_ENDPOINTS.length]}`
        );
      }
...
export async function getTokenHoldings(address: string): Promise<TokenHolding[]> {
  try {
    ...
    throw new Error(
      `Failed to fetch token holdings: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function getLastTransaction(address: string): Promise<Transaction | null> {
  try {
    ...
    throw new Error(
      `Failed to fetch last transaction: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
 ...
 */
export function countTransfersInMonth(
...
export async function countTransactionsByDateRange(
  ...
  try {
    ...
    throw new Error(
      `Failed to count transactions: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export async function getDetailedTransactions(
  ...
  try {
    console.log(
      `🔍 Fetching detailed transactions page ${page} (${limit} per page) from ${startDate} to ${endDate}`
    );
    ...
    console.log(
      `📄 Processing ${pageTransactions.length} transactions for page ${page}/${totalPages}`
    );
    ...
    throw new Error(
      `Failed to fetch detailed transactions: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
async function getTransactionDetails(signature: string): Promise<DetailedTransaction> {
  return await executeWithFallback(async (connection) => {
    ...
    const accounts =
      tx.meta?.preBalances?.map((preBalance: number, index: number) => {
    ...
    if (transfers.length === 0) {
      const significantChanges = accounts.filter(
        (account) => Math.abs(account.balanceChange) > 0.000001
      );
      ...
      senders.forEach((sender) => {
        const matchingReceiver = receivers.find(
          (receiver) => Math.abs(Math.abs(sender.balanceChange) - receiver.balanceChange) < 0.000001
        );
  ...
  try {
    ...
    try {
      ...
      if (rpcParsedInfo.value?.data && 'parsed' in rpcParsedInfo.value.data) {
        ...
        console.log(
          `  Decimals: Morpheus=${parsedTokenData.fields.decimals} | RPC=${rpcData.decimals}`
        );
        console.log(
          `  MintAuth: Morpheus=${parsedTokenData.fields.mintAuthority} | RPC=${rpcData.mintAuthority}`
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (solscanResponse.ok) {
        ...
        if (holdersData && holdersData.length > 0) {
          ...
          const decimals = rpcValidation
            ? rpcValidation.decimals
            : parseInt(parsedTokenData.fields.decimals) || 9;
          ...
          console.log(
            `📊 Using decimals: ${decimals}, raw supply: ${rawSupply}, total supply: ${totalSupply}`
          );
          ...
          const holders = holdersData.map((holder: any, index: number) => ({
            ...
            percentage:
              totalSupply > 0
                ? (parseFloat(holder.amount) / Math.pow(10, decimals) / totalSupply) * 100
                : 0,
            rank: index + 1,
          }));
    ...
    try {
      ...
      const jupiterResponse = await fetch(
        `https://quote-api.jup.ag/v6/quote?inputMint=${mintAddress}&outputMint=So11111111111111111111111111111111111111112&amount=1000000`,
        {
    ...
    console.log(
      `📊 Fallback using decimals: ${decimals}, raw supply: ${rawSupply}, total supply: ${totalSupply}`
    );
    ...
    return {
      ...
      tokenAuthorities: {
        mintAuthority: rpcValidation
          ? rpcValidation.mintAuthority
          : parsedTokenData.fields.mintAuthority,
        freezeAuthority: rpcValidation
          ? rpcValidation.freezeAuthority
          : parsedTokenData.fields.freezeAuthority,
        isInitialized: rpcValidation
          ? rpcValidation.isInitialized
          : parsedTokenData.fields.isInitialized,
      },
      recommendation:
        'For detailed holder analysis, the system uses specialized blockchain indexing services. Morpheus provides intelligent parsing with RPC validation for accuracy.',
    };
  } catch (error) {
    console.error('Failed to fetch token holders with Morpheus:', error);
    throw new Error(
      `Failed to fetch token holders: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
